 ...
 * @author Peng Mu
 */
public class TudouAnalyzerTest
{
    static{
    ...
    {
        TudouAnalyzerTest ins = new TudouAnalyzerTest();
        //ins.getWebpage();
        //ins.getProg();
        //ins.deleteProg();
        ins.testGetEpiUrl();
    ...
    {
        try{
        String webpage = WebUtil.getUnzippedWebPage("http://www.tudou.com/playlist/p/a43665i73356326.html", "gb2312", null);
        Log.info(webpage);
    ...
    private void testGetEpiUrl()
    {
        ...
        String re = WebUtil.getWebPage("http://www.tudou.com/playlist/p/a43665i73356326.html", "GB2312");
        Log.info(re);
        ArrayList<String> l = RegexUtil.getAllUniqueMatch(re, "iid:([\\d]{6,10})\\s*\\n");
        for(String s : l)
        {
            Log.info(s);
        }

        re = WebUtil.getWebPage("http://v2.tudou.com/v?it=73356326&hd=2", "GB2312");
        Log.info(re);
        String url = RegexUtil.getFirstMatchByIndex(re, ">(http:[^\\s><]+key=[^\\s><&]+)&", 1);
        Log.info(url);
        String name = RegexUtil.getFirstMatchByIndex(re, "title=\"([^\"]+)\"", 1);
        Log.info(name);
        ...
        WebUtil.downloadFile(url, "c:/temp/output.f4v", prop);
